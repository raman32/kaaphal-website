...
function CreateLoksewaCategory(): JSX.Element {
    ...
    const [createCategory] = useCreateLoksewaCategoryMutation();
    const [deleteCategory] = useDeleteLoksewaCategoryMutation();
    const [updateCategory] = useUpdateLoksewaCategoryMutation();
    //TODO fix this
    //if (!data) return <div className="flex flex-col h-full justify-center items-center"> <Spin /> Loading </div>
    const [showNewField, setShowNewField] = useState(false);
    const [form] = useForm();
    return (<div>
        { data && data.getLoksewaCategories.map(category => (
...
CreateLoksewaCategory.getLayout = (page: JSX.Element): React.ReactNode => <AdminLayout>{page}</AdminLayout>

export default CreateLoksewaCategory;