...
const LoksewaMCQQuestions = ({ params }: InferGetStaticPropsType<typeof getStaticProps>): JSX.Element => {
    ...
    return (
        <div className="w-full px-4 sm:px-8 text-center" role="application">
            ...
            {mockCategory ? <div> <h3 className="my-4" dangerouslySetInnerHTML={{ __html: mockCategory.getMockCategory.titleNP }}></h3>
                <h3 className="my-4" dangerouslySetInnerHTML={{ __html: mockCategory.getMockCategory.title }}></h3> </div> : null}
            <Divider />
            {data ? data.getMockSets.map((set, index) => <div key={index} className="shadow hover:shadow-lg mx-auto my-4 px-4 py-4 justify-around max-w-sm cursor-pointer flex flex-row" role="Button" ><div className="text-sm bg-red-100 rounded-full px-2 py-1 w-20 text-center">{set.type}</div><div className="">Set {index} </div>{set.type === MockSetType.Premium ? <DollorIcon className="align-text-top" /> : <div className="w-4" />}</div>) : <div className=" flex flex-col h-full justify-center items-center"> <Spin /> Loading </div>}
        </div >)
}
